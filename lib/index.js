...
/**
 * Create multiple attestations in batch.
 * @param {Array<{agent: string, type: string, proof: object, subject?: string}>} items
 * @returns {Array<object>} Array of unsigned attestations
 */
function createBatch(items) {
  if (!Array.isArray(items) || items.length === 0) {
    throw new Error('createBatch requires a non-empty array of attestation specs');
  }
  return items.map(item => createAttestation(item));
}

/**
 * Sign multiple attestations in batch.
 * @param {Array<object>} attestations - Array of unsigned attestations
 * @param {Buffer|string} privateKey - Ed25519 private key
 * @returns {Array<object>} Array of signed attestations
 */
function signBatch(attestations, privateKey) {
  if (!Array.isArray(attestations) || attestations.length === 0) {
    throw new Error('signBatch requires a non-empty array of attestations');
  }
  return attestations.map(a => signAttestation(a, privateKey));
}

/**
 * Verify multiple signed attestations in batch.
 * @param {Array<object>} signedAttestations
 * @param {Buffer|string} publicKey
 * @returns {{ valid: number, invalid: number, results: boolean[] }}
 */
function verifyBatch(signedAttestations, publicKey) {
  const results = signedAttestations.map(a => {
    try { return verifyAttestation(a, publicKey); }
    catch { return false; }
  });
  return {
